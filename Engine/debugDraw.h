...
namespace Debug
{
	...
	extern "C"
	{
		...
		ENGINE_API void DrawCross(const Vector2D& position, const Vector2D& size, const Color& color = Color::red);
#ifdef __cplusplus
	}
#endif
}
	...
	#define DEBUG_DRAW_CROSS(position, size, color) Debug::DrawCross(position, size, color);
#else
	#define DEBUG_DRAW_AABB(aabb, color);
	#define DEBUG_DRAW_LINE(v1, v2, color);
	#define DEBUG_DRAW_CROSS(position, size, color);
#endif